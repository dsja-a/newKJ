...
class FeishuChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        def run_ws():
            ...
            try:
                while self._running:
                    try:
                        logger.info("飞书 WebSocket 正在连接... (domain={})", domain)
                        self._ws_client.start()
                        # start() returns only on disconnect/error
                        logger.warning("飞书 WebSocket 连接断开，将在 5 秒后重连...")
                    except Exception as e:
                        logger.warning("飞书 WebSocket 连接失败: {} (5 秒后重试)", e)
                    if self._running:
                        time.sleep(5)
            finally:
                ws_loop.close()
                logger.info("飞书 WebSocket 线程已关闭")
    ...
    def _on_message_sync(self, data: Any) -> None:
        ...
        """
        self._event_count += 1
        # 每次收到底层事件都打印，确认 lark SDK 是否正确分发了事件
        logger.info(
            "飞书底层事件 #{}, type={}, loop_ok={}",
            self._event_count,
            type(data).__name__,
    ...
    async def _on_message(self, data: P2ImMessageReceiveV1) -> None:
        ...
        try:
            ...
            # Log first successful message receipt — proves WebSocket is working
            if not self._first_message_logged:
                self._first_message_logged = True
                logger.info(
                    "✅ 飞书 WebSocket 连接成功！收到第一条消息 (sender={}, chat_type={})",
